...
#########################################
# m4c
# Note: the only difference between this model and M4b is that here, the blasts are independent of WBCs (i.e. they're not inhibited).
...
param_names_m4c = ['ktr', 'gam', 'slope_ven_blast', 'slope_aza_blast', 'slope_ven_wbc', 'slope_aza_wbc', 'B', 'Xl1', 'B0', 'p1', 'bi']
param_bounds_m4c = [(0, 1), (0, 1), (0, 2), (0, 2), (0, 2), (0, 2), (0, 10), (0, 10), (0, 10), (0, 1), (0, 1)]

def initialization_fn_m4c(model, param_vals):
    "Initial WBC is a free parameter"
    ktr = param_vals[0]
    B = param_vals[6]
    B0 = param_vals[8]
...
def initialization_fn_m4c_2(model, param_vals):
    "Initial WBC is a free parameter"
    ktr = param_vals[0]
    B = param_vals[6]
    B0 = param_vals[8]
...
def build_pm_model_m4c(model, wbc, blasts, theta=None, n_samples=None, params_to_fit=None,
        initialization=None, use_b0=True, uniform_prior=False, **params):
    """
    Builds a PyMC model
    """
    if params_to_fit is None:
        params_to_fit = param_names_m4c
    if theta is None:
        model.resetToOrigin()
        default_params = [model.getValue(x) for x in params_to_fit]
        theta = default_params
